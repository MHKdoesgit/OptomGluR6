...
m1          = max(m1,1e-3); 
m2          = min(m2,-1e-3);
tg1         = max(dt,abs(tt(idx1))); 
tg2         = max(dt,abs(tt(idx2)));

%nguess=max([1 log(10/m1) log(10/abs(m2))]);
%guess=[100 tg1 100 tg2 nguess];
guess       = [m1 tg1 abs(m2) tg2 5];
pp          = polyfit(rfroutines.templowpass(guess,tt),ysmooth,1);
guess       = [m1*pp(1) tg1 abs(m2)*pp(1) tg2 5];

[~,startGrad]   = rfroutines.templowpass(guess,tt); %calculate gradient at initial point
...
guessparams = lsqcurvefit(@rfroutines.templowpass, guess, tt, ysmooth,lb,ub,guessopts);
%==========================================================================
[~,startGrad] = rfroutines.templowpass(guessparams,tt); %calculate gradient at initial point
if all(isfinite(startGrad)); useGrad=true; else, useGrad=false; end
...
opts        = optimoptions('lsqcurvefit','Display','off',...
            ...
            'SpecifyObjectiveGradient',useGrad,'CheckGradient',false);
params      = lsqcurvefit(@rfroutines.templowpass, guessparams, tt, yy,lb,ub,opts);
%==========================================================================
end
