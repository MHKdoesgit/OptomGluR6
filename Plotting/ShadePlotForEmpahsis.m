...
% fixing bug for multiple vectors and matching correct edge colors
% updated by Mohammad 14.06.2015
...
for i=1:length( x)
    h(i)=patch([repmat( x{i}(1),1,2) repmat( x{i}(end),1,2)], ...
        [get(gca,'YLim') fliplr(get(gca,'YLim'))], ...
        [0 0 0 0],colors{i});
    set(h(i),'FaceAlpha',alphas{i});
    set(h(i),'EdgeColor',colors{i});   % set the Edge color to the patch color
    set(h(i),'EdgeAlph',0);
    %set(h(i),'FaceVertexAlphaData',alphas{i});    % Set constant transparency
    %set(h(i),'FaceAlpha','flat') ;          % Interpolate to find face transparency
    
end;
set(gca ,'Layer', 'Top'); % bring the axis to front
